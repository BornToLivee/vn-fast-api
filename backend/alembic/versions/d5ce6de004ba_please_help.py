...
def upgrade() -> None:
    ...
    op.execute("CREATE TYPE novelstatus AS ENUM ('READING', 'COMPLETED', 'DROPPED', 'PLANNING')")
    op.execute("CREATE TYPE novellanguage AS ENUM ('RUSSIAN', 'ENGLISH', 'UKRAINIAN', 'OTHER')")
    ...
    op.execute("ALTER TABLE novels ALTER COLUMN status TYPE novelstatus USING status::novelstatus")
    op.execute("ALTER TABLE novels ALTER COLUMN language TYPE novellanguage USING language::novellanguage")
